...
GLShader::GLShader(ShaderType shaderType, const std::string& name, ResourceListener* listener) :  Shader(shaderType, name, listener)
{
...
GLShader::~GLShader()
{
	//je¿eli shader nie zosta³ usuniêty poprzez funkcjê unload
	if (m_id > 0)
	{
		glDeleteShader(m_id);
	}
